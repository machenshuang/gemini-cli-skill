...
Claude will issue the Bash call with `run_in_background: true` and give you a task ID to track progress.
...
Claude will issue multiple background Bash calls in a single response — one per task.

## Task Management

Once a background task is running, use these commands to manage it:
